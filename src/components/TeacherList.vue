            ...
            <section class="lg:grid-cols-2 gap-3 grid">
                <li v-for="(person, index) in people" :key="index" class="mb-2">
                    <div class="border p-4 shadow-lg rounded-lg ">
                        <p class="mb-2"><span class="font-medium">Name</span>: {{ person.title }} {{ person.name }} {{
                            person.surname }}
                        </p>
                        <p class="mb-2"><span class="font-medium">Contact</span>: {{ person.teacherNumber }}</p>
                        <p class="mb-2"><span class="font-medium">National ID</span> {{ person.nationalId }}</p>
                        <div class="flex justify-between">
                            <p><span class="font-medium">DOB:</span> {{ person.dob }}</p>
                            <p v-if="person.salary"><span class="font-medium">Salary:</span> &#8358;{{ person.salary }}
                            </p>
                        </div>
                        <div class="flex gap-3 mt-3 justify-end">
                            <BasicButton kind="grey" label="Edit" @click="editPerson(index)" />
                            <BasicButton kind="danger" label="Delete" @click="deletePerson(index)" />
                        </div>
                    </div>
                </li>
            </section>
