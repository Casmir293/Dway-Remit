    ...
    <div class="mb-4">
        ...
        <form @submit.prevent="submitForm">
            <div class="grid grid-cols-2 gap-4">
                ...
                <input v-model="student.studentNumber" placeholder="Student Number" required>
            </div>
            <button type="submit" class="mt-2 p-2 bg-green-500 text-white">Save Student</button>
        </form>
    </div>
</template>
...
const student = ref({ nationalId: '', name: '', surname: '', dob: '', studentNumber: '' });

const submitForm = () => {
    const age = new Date().getFullYear() - new Date(student.value.dob).getFullYear();
    if (age > 22) {
    ...
    // Clear the form
    student.value = { nationalId: '', name: '', surname: '', dob: '', studentNumber: '' };
};
</script>
